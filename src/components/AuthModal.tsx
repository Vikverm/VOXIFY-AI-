  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        {/* Modal Header */}
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-start justify-between bg-gradient-to-b from-slate-50 to-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 tracking-tight flex items-center gap-1.5">
                {mode === "login" ? "Client Sign In" : "Register Client Account"}
                <Sparkles className="h-4 w-4 text-blue-600" />
              </h2>
              <p className="text-xs text-slate-500">
                {mode === "login"
                  ? "Access your saved audio takes, API keys & plan credits"
                  : "Get 10,000 Free Credits instantly on sign up"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 pt-4 pb-1">
          <div className="grid grid-cols-2 p-1 rounded-xl bg-slate-100 border border-slate-200/80 text-xs font-semibold">
            ...
            >
              <span>Register</span>
              <span className="text-[10px] px-1.5 py-0.2 bg-blue-50 text-blue-700 rounded-full font-bold">
                +50k
              </span>
            </button>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {mode === "register" && (
              <div className="p-3 rounded-xl bg-blue-50/70 border border-blue-100 flex items-center gap-2.5 text-xs text-blue-900">
                ...
                <span>
                  <strong>10,000 Free Credits:</strong> Ready immediately for high-fidelity speech synthesis, Voice Changer &amp; studio tools.
                </span>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-xs transition cursor-pointer"
            >
              {isLoading ? (
                <>
          ...
          <div className="relative flex py-2 items-center">
            ...
            <span className="shrink mx-3 text-slate-400 text-xs">or continue with</span>
            <div className="grow border-t border-slate-200"></div>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2.5 px-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs flex items-center justify-center gap-2.5 shadow-2xs transition cursor-pointer"
          >
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
          <div className="flex items-center gap-1.5">
            ...
            <span>256-bit Cloud Security &amp; Data Privacy</span>
          </div>
          <button
            type="button"
